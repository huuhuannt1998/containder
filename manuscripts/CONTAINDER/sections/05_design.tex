...
issuance, fresh-key binding contains a copied key, scope bounds the commandable set and a short
lifetime bounds $T_{\mathrm{cred}}$, session and command enforcement bound $T_{\mathrm{sess}}$
and $T_{\mathrm{cmd}}$, and measured recovery bounds the feeder consequence.}
\label{fig:chain}
\end{figure*}
...
\subsection{Value-constrained authorization, and where it is enforced}
% provenance: STANDARDS_CLAIM_MATRIX.md S7.
The physical result of Section~\ref{sec:results} is a statement about the \emph{set} of reactive
operating points an authorization admits, so the design is only actionable if that set can be
expressed and enforced. \texttt{FunctionSetAssignments} scopes which \emph{functions} an identity
may invoke. No accessible source establishes that IEEE~2030.5 or CSIP can attach a bound on the
\emph{value} a control may take, such as ``reactive output must remain at or below
$-\varphi$\,kvar'' (row S7, Table~\ref{tab:standards}).

We therefore make the weaker and checkable claim. CONTAINDER's numeric authorization is a
\textbf{layer above \texttt{FunctionSetAssignments}, enforced by the DERMS or the gateway proxy},
not a native protocol capability. The enforcement point holds the bound, rejects or clamps an
out-of-set command before forwarding it, and emits only conformant IEEE~2030.5 downstream, so
protocol compatibility is preserved and the constraint is applied where the credential is
terminated. Whether a future profile could carry the bound natively is a question for the working
group, and we do not assume the answer.
...
\end{align}
each targeted to zero or a small declared bound. These are the $T_{\mathrm{sess}}$ and
$T_{\mathrm{cmd}}$ components of the retained-authority vector introduced in
...
Section~\ref{sec:results} reports interoperability as untested. Three tiers realize this: a
...
attestation}. Per-epoch fresh-key binding answers exactly that class, and it answers it
\emph{without} attestation: because renewal mints a fresh key whether or not anything is
detected, a copied key is inert after the epoch regardless of the detection probability, and
expected retention is $T/2$ under uniform arrival. What attestation bounds is the different class
in which the adversary remains resident on the device and re-attests successfully; there
retention is $T(1/p - 1/2)$ for uniform arrival, and the familiar $T/p$ only when the compromise
lands immediately after an issuance. Table~\ref{tab:classes} separates them, because reporting one
figure for both --- as an earlier version of this paper did --- credits attestation with a bound
key rotation already supplies and, at $T{=}6$\,h and $p{=}0.2$, overstates copied-key retention
tenfold. The alternative, a narrow access list on an indefinitely valid credential, contains a
copied key only when someone notices and edits the list, which has no bounded latency at all.
Bounded beats unbounded; it does not beat instant.
...
TLS, with a Python IEEE~2030.5-style client in place of a vendor client, and structured event
logging for every credential, session, and command transition. Its security invariants and
overhead are reported in Section~\ref{sec:results}. Two components of the design are specified and
enforced in the prototype but not exercised against external hardware or software: the operational
credential is not held in a hardware keystore, and no third-party IEEE~2030.5 endpoint has
consumed the gateway's downstream traffic. Section~\ref{sec:limits} states what each absence
bounds.
