...
these are varied in the evaluation. On a synthetic 200-device fully-central fleet spread over four
feeders, the reachable-DER count grows from one (single-device) to one (site) to fifty (feeder) to
two hundred (fleet), and commandable capacity grows in step. The feeder-level figure is the
fleet divided by the feeder count, so it moves with that generator parameter; the parameters are
released with the artifact. This decomposition operationalizes RQ1: most of the reach
